...
import { classifySeverity, getRecommendations } from '@/lib/ai/recommendations';
...
export default function MotionRecorder() {
  ...
  const computeAndRender = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (s.length === 0) {
        graspCyclesPerFinger.push(0);
        graspQualityPerFinger.push(0);
        continue;
      }
    ...
    const ax = accelSeriesRef.current.x,
      ay = accelSeriesRef.current.y,
      az = accelSeriesRef.current.z;
    ...
    let tremorScore = 0;
    if (tremorHz >= 3 && tremorHz <= 7) tremorScore = 70;
    else if (tremorHz > 7) tremorScore = 40;
    else tremorScore = 20;
    const midIdx = 2;
    ...
    try {
      ...
      analysisRecordService.saveRecord({
        ...
        source: (connectionType as 'arduino' | 'web-analysis' | 'manual') || 'manual',
  ...
  return (
    <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Motion & Sensor Analysis</h2>
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
          ...
          >
            {isCollecting ? 'Collecting (10s)...' : 'Start Motion Analysis (10s)'}
          </Button>
        </div>
